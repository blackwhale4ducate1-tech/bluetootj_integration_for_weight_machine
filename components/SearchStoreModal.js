...
export default function SearchStoreModal({
  modalTitle,
  showModal,
  handleClose,
  updateFormData,
    ...
    async inputValue => {
      try {
        const res = await axios.post(
          `${API_BASE_URL}/api/searchStoresByInput`,
          {
  ...
  useEffect(() => {
    ...
    setSearchInput(storeInput); // Clear the search input when modal opens or closes
    if (showModal && inputRef.current) {
      inputRef.current.focus(); // Focus the input field when the modal is opened
      onHandleSearchInput(storeInput);
    }
  }, [showModal, storeInput, onHandleSearchInput]);
  ...
  const handleSubmit = () => {
    updateFormData(prevFormData => ({
      ...prevFormData,
      store_name: searchResults[activeRow]?.store_name || '',
    }));
    handleClose();
  };
  ...
  const tableData = searchResults.map(result => [result.store_name]);
