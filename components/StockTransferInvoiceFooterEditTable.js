...
const StockTransferInvoiceFooterEditTable = ({
  invoice_no,
  formData,
  updateFormData,
  totalAmt,
  ...
  useEffect(() => {
    const getInvoiceFooterData = async company_name => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/getStockTransferFooterData?invoiceNo=${invoice_no}&company_name=${company_name}`,
        );
...
export default StockTransferInvoiceFooterEditTable;
