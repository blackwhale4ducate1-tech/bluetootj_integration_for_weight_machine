...
const SearchProductCategoryForStockMenu = ({
  updateFormData,
  ...
  const [searchInput, setSearchInput] = useState(productCategoryInput);
  ...
  const onHandleSearchInput = async text => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/searchProductCategoryByInput`,
        {
          searchInput: text,
  ...
  const handleClickRow = result => {
    ...
    updateFormData(prevFormData => ({
      ...prevFormData,
      product_category_name: result.name,
      product_code: '',
      barcode: '',
    }));
    ...
    <View>
      <Menu
        ...
        <ScrollView style={{maxHeight: 200, width: '100%'}}>
          ...
          {searchResults.length === 0 && (
            <Text style={styles.emptyprd}>No Product Category found</Text>
          )}
        </ScrollView>
      </Menu>
    </View>
...
export default SearchProductCategoryForStockMenu;
