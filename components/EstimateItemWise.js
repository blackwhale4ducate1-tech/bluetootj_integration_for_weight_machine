...
export default function EstimateItemWise({formData}) {
  const {data} = useAuth();
  ...
  useEffect(() => {
    const getEstimateItemWiseData = async company_name => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/getEstimateItemWiseData?fromDate=${fromDateStr}&toDate=${toDateStr}&productCode=${encodedProductCode}&username=${encodedUserName}&company_name=${company_name}`,
        );
    ...
    getEstimateItemWiseData(data.company_name);
