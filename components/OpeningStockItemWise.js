...
export default function OpeningStockItemWise({formData}) {
  const {data} = useAuth();
  ...
  useEffect(() => {
    const getOpeningStockItemWiseData = async company_name => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/getOpeningStockItemWiseData?fromDate=${fromDateStr}&toDate=${toDateStr}&productCode=${encodedProductCode}&company_name=${company_name}`,
        );
    ...
    getOpeningStockItemWiseData(data.company_name);
