...
export default function SalesOrderDayBook({formData}) {
  const {data} = useAuth();
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${API_BASE_URL}/api/getSalesOrderDayBookData?fromDate=${fromDateStr}&toDate=${toDateStr}&customerName=${encodedCustomerName}&username=${encodedUserName}&storeName=${encodedStoreName}&status=${formData.status.value}&company_name=${data.company_name}`;
  }, [
    formData.fromDate,
    formData.toDate,
    formData.name,
  ...
  const handleRowSelection = invoice_no => {
    if (selectedInvoiceNumbers.includes(invoice_no)) {
      // If the invoice number is already selected, deselect it
      setSelectedInvoiceNumbers(prevSelected =>
        prevSelected.filter(invoice => invoice !== invoice_no),
      );
    } else {
      // If the invoice number is not selected, select it
      setSelectedInvoiceNumbers(prevSelected => [...prevSelected, invoice_no]);
    }
  };
  ...
  const handleCheckAllChange = () => {
    ...
    if (!selectAll) {
      // If not all are selected, select all invoice numbers
      const allInvoiceNumbers = dayBookData
        .filter(row => row.status !== 'Completed')
        .map(row => row.invoice_no);
      setSelectedInvoiceNumbers(allInvoiceNumbers);
    } else {
      // If all are selected, deselect all invoice numbers
      setSelectedInvoiceNumbers([]);
    }
  };

  const handleOnClickEdit = invoice_no => {
    navigation.navigate('SalesOrderEditBilling', {invoice_no: invoice_no});
  };
      ...
      (acc, row) => {
        acc.cash +=
          row.mop === 'CASH'
            ...
            : 0;
        acc.card += row.card;
        acc.credit += row.mop === 'CREDIT' ? row.total_amount : 0;
        ...
        <>
                  ...
                  <ScrollView
                    ...
                    <View style={{marginBottom: 100}}>
                      ...
                      <DataRow
                        label="Cash"
                        value={
                          selectedRow.mop === 'CASH'
                            ? selectedRow.total_amount
                            : selectedRow.mop === 'MIXED'
                            ? selectedRow.cash
                            : 0
                        }
                        iconComponent={
          ...
          <View style={styles.childView}>
            <View style={styles.childWrapper}>
              <Text style={styles.cardText}>Total Cash : </Text>
              <Text style={styles.cardValues}>{totalSum.cash.toFixed(2)}</Text>
            </View>
            <View style={styles.childWrapper}>
              <Text style={styles.cardText}>Total Card : </Text>
              <Text style={styles.cardValues}>{totalSum.card.toFixed(2)}</Text>
            </View>
            <View style={styles.childWrapper}>
              <Text style={styles.cardText}>Total Credit : </Text>
              <Text style={styles.cardValues}>
                {totalSum.credit.toFixed(2)}
              </Text>
            </View>
            <View style={styles.childWrapper}>
