...
export default function PurchaseReturnDayBook({formData}) {
  const {data} = useAuth();
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${API_BASE_URL}/api/getPurchaseReturnDayBookData?fromDate=${fromDateStr}&toDate=${toDateStr}&customerName=${encodedCustomerName}&company_name=${data.company_name}`;
  ...
  const handleOnClickEdit = invoice_no => {
    navigation.navigate('PurchaseReturnEditBilling', {invoice_no: invoice_no});
  };
                ...
                <View key={`row-${startIndex + index}`}>
                  <View style={styles.reportView}>
                    ...
                    <View
                      ...
                      />
                      <Text style={[styles.cardValues, {paddingLeft: 7}]}>
                        <Text style={styles.cardText}>Vendor Name :</Text>{' '}
                        {row.vendor_name}
                      </Text>
                    </View>
                    <View
