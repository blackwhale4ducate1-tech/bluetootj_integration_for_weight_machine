...
export default function EstimateDayBook({formData}) {
  const {data} = useAuth();
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${API_BASE_URL}/api/getEstimateDayBookData?fromDate=${fromDateStr}&toDate=${toDateStr}&customerName=${encodedCustomerName}&username=${encodedUserName}&storeName=${encodedStoreName}&company_name=${data.company_name}`;
  }, [
    formData.fromDate,
    formData.toDate,
    formData.name,
  ...
  const handleOnClickEdit = invoice_no => {
    navigation.navigate('EstimateEditBilling', {invoice_no: invoice_no});
  };
