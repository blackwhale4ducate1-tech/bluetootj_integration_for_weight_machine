...
export default function StockTransferItemWise({formData}) {
  const {data} = useAuth();
  ...
  useEffect(() => {
    const getStockTransferWiseData = async company_name => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/getStockTransferItemWiseData?fromDate=${fromDateStr}&toDate=${toDateStr}&productCode=${encodedProductCode}&company_name=${company_name}`,
        );
    ...
    getStockTransferWiseData(data.company_name);
