...
export default function ReceiptReportTable({formData}) {
  const {data} = useAuth();
  ...
  useEffect(() => {
    const getReceiptReportData = async company_name => {
      try {
        const fromDateStr = formData.fromDate.toISOString().split('T')[0];
        const toDateStr = formData.toDate.toISOString().split('T')[0];
        const response = await axios.get(
          `${API_BASE_URL}/api/getReceiptReportData?fromDate=${fromDateStr}&toDate=${toDateStr}&company_name=${company_name}`,
        );
    ...
    getReceiptReportData(data.company_name);
