...
const SearchBarcodeForStockMenu = ({updateFormData, barcodeInput}) => {
  const {data} = useAuth();

  const [menuVisible, setMenuVisible] = useState(false);
  const [searchResults, setSearchResults] = useState([]);
  const [searchInput, setSearchInput] = useState(barcodeInput);
  ...
  const onHandleSearchInput = async text => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/searchBarcodeByInputForStocksReport`,
        {
          searchInput: text,
...
export default SearchBarcodeForStockMenu;
