...
export default function PettySalesItemWise({formData}) {
  const {data} = useAuth();
  ...
  useEffect(() => {
    const getPettySalesItemWiseData = async company_name => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/getPettySalesItemWiseData?fromDate=${fromDateStr}&toDate=${toDateStr}&productCode=${encodedProductCode}&username=${encodedUserName}&company_name=${company_name}`,
        );
    ...
    getPettySalesItemWiseData(data.company_name);
      ...
      <View style={[styles.childView]}>
        ...
        <View style={styles.childWrapper}>
          ...
          <Text style={styles.cardValues}>
            {totalSum.sales_price.toFixed(2)}
          </Text>
        </View>
      </View>
    </View>
