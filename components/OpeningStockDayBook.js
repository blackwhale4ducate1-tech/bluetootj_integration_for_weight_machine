...
export default function OpeningStockDayBook({formData}) {
  const {data} = useAuth();
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${API_BASE_URL}/api/getOpeningStockDayBookData?fromDate=${fromDateStr}&toDate=${toDateStr}&customerName=${encodedCustomerName}&username=${encodedUserName}&storeName=${encodedStoreName}&company_name=${data.company_name}`;
  }, [
    formData.fromDate,
    formData.toDate,
    formData.name,
  ...
  const handleOnClickEdit = invoice_no => {
    navigation.navigate('OpeningStockEditBilling', {invoice_no: invoice_no});
  };
