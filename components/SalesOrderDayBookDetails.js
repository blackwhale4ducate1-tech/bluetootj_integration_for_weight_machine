...
export default function SalesOrderDayBookDetails({formData}) {
  const {data} = useAuth();
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${API_BASE_URL}/api/getSalesOrderDayBookDetailData?fromDate=${fromDateStr}&toDate=${toDateStr}&customerName=${encodedCustomerName}&status=${formData.status.value}&company_name=${data.company_name}`;
  }, [
    formData.fromDate,
    formData.toDate,
    formData.name,
