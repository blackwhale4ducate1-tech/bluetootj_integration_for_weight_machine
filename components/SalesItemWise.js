...
export default function SalesItemWise({formData}) {
  const {data} = useAuth();
  ...
  useEffect(() => {
    const getSalesItemWiseData = async company_name => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/getSalesItemWiseData?fromDate=${fromDateStr}&toDate=${toDateStr}&productCode=${encodedProductCode}&username=${encodedUserName}&company_name=${company_name}`,
        );
    ...
    getSalesItemWiseData(data.company_name);
