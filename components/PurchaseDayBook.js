...
export default function PurchaseDayBook({formData}) {
  const {data} = useAuth();
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${API_BASE_URL}/api/getPurchaseDayBookData?fromDate=${fromDateStr}&toDate=${toDateStr}&customerName=${encodedCustomerName}&company_name=${data.company_name}`;
  ...
  const handleOnClickEdit = invoice_no => {
    navigation.navigate('PurchaseEditBilling', {invoice_no: invoice_no});
  };
