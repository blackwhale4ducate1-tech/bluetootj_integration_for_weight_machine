...
const PurchaseInvoiceItemsEditMenu = ({type, invoice_no, setItemsLoading}) => {
  const {data} = useAuth();
  if (type === 'purchase') {
    ({
      items,
              ...
              onPress={() => {
                ...
                navigation.navigate(
                  type === 'purchase_return'
                    ? 'AddEditPurchaseReturnItem'
                    : 'AddEditPurchaseItem',
                  {type: type, invoice_no: invoice_no},
                );
              }}>
              <Card
                key={index}
...
export default PurchaseInvoiceItemsEditMenu;
