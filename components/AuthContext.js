  ...
  useEffect(() => {
    ...
    if (!data && jwt) {
      axios
        ...
        .then(res => {
          if (res.data.error) {
            console.log('error in auth: ' + res.data.error);
            navigation.navigate('Login');
          }
          console.log('isAuthenticated in AuthContext: ' + isAuthenticated());
          updateData(res.data.userData); // Set data in AuthProvider
          updatePermissions(res.data.permissionsData);
        })
        .catch(err => {
          console.log(err);
        });
