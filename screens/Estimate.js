...
const Estimate = () => {
  ...
  const templateType =
    data.is_barcode_exist === 1
      ? 'estimate_items_with_barcode'
      : 'estimate_items_without_barcode';
  ...
  useEffect(() => {
    ...
    if (data.defaultAccountCode !== '') {
      getCustomerData(data.company_name, data.defaultAccountCode);
    }
  }, [data.company_name, data.defaultAccountCode]);
  ...
  const getMOP = async (username, company_name) => {
    try {
      ...
      setFormDataFooter(selectedOption => ({
        ...selectedOption,
        mop: {value: response.data.estimate, label: response.data.estimate},
        invoice_print_type: {
          value: response.data.estimate_invoice_type,
          label: response.data.estimate_invoice_type,
        },
        estimate_account_add_status: {
          value: response.data.estimate_accounts_add_status,
          label:
            response.data.estimate_accounts_add_status === 1
              ? 'Enable'
              : 'Disable',
        },
      }));
  ...
  const onClickSubmit = async type => {
    ...
    if (
      ...
      (!formDataFooter.mop &&
        formDataFooter.estimate_account_add_status.value === 1) ||
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/addEstimateInvoice`, {
        type: '',
        invoiceNo: '',
      ...
      if (res.data.message) {
        newEstimate();
        const pageTitle = 'Estimate Invoice';
        let pageSize;
        ...
        const invoiceHTML = GenerateInvoicePrint({
          ...
          addEstimateAddress: data.estimate_invoice_header_address,
          data,
        ...
        if (type === 'share') {
          const options = {
            html: invoiceHTML,
            fileName: 'estimate_invoice',
            directory: 'Documents',
          };
          const file = await RNHTMLtoPDF.convert(options);
          await Share.open({
            title: 'Share Estimate Invoice',
  ...
  return (
    <View>
      <ScrollView contentContainerStyle={{backgroundColor: COLORS.white}}>
        ...
        <InvoiceHeaderTable type="estimate"></InvoiceHeaderTable>
        <SalesInvoiceItemsMenu type="estimate" />
        <InvoiceFooterForAll
          type="estimate"
          handleTaxChange={handleTaxChange}
          handlePriceListChange={handlePriceListChange}
          invoiceFooterType="EstimateInvoiceFooterTable"></InvoiceFooterForAll>
