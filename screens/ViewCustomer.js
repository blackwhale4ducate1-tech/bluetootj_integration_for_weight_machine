...
const ViewCustomer = () => {
  ...
  const isEditCustomer = permissions.find(
    permission => permission.page === 'customers' && permission.edit === 1,
  );
  const isDelCustomer = permissions.find(
    permission => permission.page === 'customers' && permission.del === 1,
  );
  ...
  const getInvoiceNo = useCallback(async company_name => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/getInvoiceSeriesByType?series_name=customer&company_name=${company_name}`,
      );
  ...
  useFocusEffect(
    useCallback(() => {
      getProductById(id);
      if (data.business_category === 'FlowerShop' && type === 'Add Customer') {
  ...
  const handleSubmitAlert = useCallback(
    ...
    [formData, handleCloseModal, data.company_name, data.business_category],
  );
        ...
        <Card style={{marginVertical: 10, backgroundColor: COLORS.white }}>
          <Card.Content>
            ...
            <View style={CustomStyles.row}>
              ...
              <Text style={[CustomStyles.boldText, styles.blackText]}>{formData.invoiceDate}</Text>
            </View>
            <View style={[CustomStyles.row, {marginTop: 20}]}> 
            ...
            <View style={CustomStyles.row}>
              ...
              <Text style={[CustomStyles.boldText, styles.blackText]}>{formData.credit_limit}</Text>
            </View>
          </Card.Content>
        </Card>
      </ScrollView>
