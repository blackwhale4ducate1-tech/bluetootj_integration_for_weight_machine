...
const PurchaseReturn = () => {
  ...
  const getProductItemForFlowerShop = async (
    business_category,
    company_name,
  ) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/getProductDetails?business_category=${business_category}&company_name=${company_name}`,
      );
      if (response.data.length > 0) {
        console.log(
          'getProductItemForFlowerShop products: ' +
  ...
  useEffect(() => {
    if (data.business_category === 'FlowerShop') {
      getProductItemForFlowerShop(data.business_category, data.company_name);
    }
  }, [data.business_category, data.company_name]);

  const getMOP = async (username, company_name, business_category) => {
    try {
      const response = await axios.get(
      ...
      setFormDataFooter(selectedOption => ({
        ...selectedOption,
        mop:
          business_category === 'FlowerShop'
            ? {value: 'CREDIT', label: 'CREDIT'}
            : {value: response.data.purchase, label: response.data.purchase},
        invoice_print_type: {
  ...
  useEffect(() => {
    getMOP(data.username, data.company_name, data.business_category);
  }, [data.username, data.company_name, data.business_category]);
  ...
  const onClickSubmit = async type => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/addPurchaseReturnInvoice`,
        {
          type: '',
          invoiceNo: '',
      ...
      if (res.data.message) {
        newPurchaseReturn();
        const pageTitle = 'Purchase Return Invoice';
        let pageSize;
        ...
        if (type === 'share') {
          const options = {
            html: invoiceHTML,
            fileName: 'purchaseReturn_invoice',
            directory: 'Documents',
          };
          const file = await RNHTMLtoPDF.convert(options);
          await Share.open({
            title: 'Share Purchase Return Invoice',
  ...
  return (
    <View>
      <ScrollView contentContainerStyle={{backgroundColor: COLORS.white}}>
        ...
        <InvoiceHeaderTable type="purchase_return"></InvoiceHeaderTable>
        <PurchaseInvoiceItemsMenu type="purchase_return" />
        <InvoiceFooterForAll
          type="purchase_return"
          handleTaxChange={handleTaxChange}
          invoiceFooterType="PurchaseInvoiceFooterTable"></InvoiceFooterForAll>
