...
import SearchPurchaseProductMenu from '../components/SearchPurchaseProductMenu';
import {useAuth} from '../components/AuthContext';
import SearchBarcodeMenu from '../components/SearchBarcodeMenu';
...
const AddEditPurchaseItem = () => {
  ...
  const {type, product_type = '', invoice_no} = route.params;
  ...
  const handleSaveItem = () => {
    ...
    setSearchProductInput('');
    navigation.navigate(billingType, {invoice_no: invoice_no});
  };
  ...
  return (
    <View style={{flex: 1, backgroundColor: COLORS.white}}>
      <Appbar.Header style={CustomStyles.appHeader}>
        ...
        />
        <Appbar.Content
          title="AddEditPurchaseItem"
          titleStyle={CustomStyles.titleStyle}
        />
      </Appbar.Header>
      <ScrollView>
        <View style={{padding: 5}}>
          <SearchPurchaseProductMenu
            type={type}
            product_type={type === 'production' ? product_type : undefined}
          />
          ...
          {data.is_barcode_exist === 1 && (
            <SearchBarcodeMenu
              type={type}
              barcodeOptions={newProduct.dropdownOptions}
              product_type={type === 'production' ? product_type : undefined}
            />
          )}
