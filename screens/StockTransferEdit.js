...
import StockTransferInvoiceHeaderEditTable from '../components/StockTransferInvoiceHeaderEditTable';
...
import StockTransferItemsEditMenu from '../components/StockTransferItemsEditMenu';
...
const StockTransferEdit = () => {
  ...
  useEffect(() => {
    setBillingType('stockTransferEdit');
    // console.log('invoice_no in StockTransferEdit: ' + invoice_no);
  ...
  const onClickSubmit = async () => {
    if (
      ...
      !formDataHeader.source ||
      !formDataHeader.destination ||
      !formDataFooter.tax
    ) {
    ...
    if (formDataHeader.source === formDataHeader.destination) {
      setValidationError('Source and Destination should not be same.');
      setShowErrorModal(true);
      return;
    }
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/addStockTransferInvoice`,
        {
          type: 'edit',
          invoiceNo: invoice_no,
  ...
  return (
    <View>
      <ScrollView contentContainerStyle={{backgroundColor: COLORS.white}}>
        ...
        <StockTransferInvoiceHeaderEditTable invoice_no={invoice_no} />
        <StockTransferItemsEditMenu invoice_no={invoice_no} />
        <InvoiceFooterForAll
          type="stock_transfer"
          invoice_no={invoice_no}
          handleTaxChange={handleTaxChange}
          handlePriceListChange={handlePriceListChange}
          invoiceFooterType="StockTransferInvoiceFooterEditTable"></InvoiceFooterForAll>
