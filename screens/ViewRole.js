...
const ViewRole = () => {
  ...
  const isEditRole = permissions.find(
    permission => permission.page === 'roles' && permission.edit === 1,
  );
  const isDelRole = permissions.find(
    permission => permission.page === 'roles' && permission.del === 1,
  );
  ...
  const getRoleById = async id => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/getRoleById?id=${id}&company_name=${data.company_name}`,
      );
  ...
  const handleSubmitAlert = useCallback(
    async buttonValue => {
      if (buttonValue === 'Yes') {
        ...
        try {
          const res = await axios.post(`${API_BASE_URL}/api/delRole`, {
            id: formData.id,
        ...
        <Card style={{marginVertical: 10, backgroundColor: COLORS.white}}>
          <Card.Content>
            <View style={CustomStyles.row}>
              <Text style={[CustomStyles.flexText, styles.blackText]}>Role</Text>
            </View>
            <View style={CustomStyles.row}>
              <Text style={[CustomStyles.boldText, styles.blackText]}>{formData.role}</Text>
            </View>
          </Card.Content>
        </Card>
      </ScrollView>
